...
public class WeightMap<T>
{
  ...
  public Set<T> keySet(){return this.item_weights.keySet();}
  public void put(T item, int weight){this.item_weights.put(item, weight);}
  public void remove(T item){this.item_weights.remove(item);}
  public boolean containsKey(T item){return this.item_weights.containsKey(item);}
  public int getWeight(T item){return this.item_weights.get(item);}
  ...
  public T getRandom()
  {
    ...
    for(T i : items)
    {
      ...
      if(selected <= i_weight) return i;
      else selected -= i_weight;
    }
    return null;
  }
  ...
  {
    int sum = 0;
    for(int i : item_weights.values()) sum += i;
    return sum;
  }
}
