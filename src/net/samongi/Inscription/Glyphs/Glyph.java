...
public class Glyph implements Serializable
{
  ...
  private static void log(String message){Inscription.log("[Glyph] " + message);;}
  @SuppressWarnings("unused")
  private static void logDebug(String message){if(Inscription.debug()) Inscription.log("[DEBUG] " + message );}
  @SuppressWarnings("unused")
  ...
  {
    if(item == null) return null; // If the passed object is null, it can't be a glyph
    ItemMeta item_meta = item.getItemMeta();
    if(item_meta == null) return null; // If the item doesn't have meta data, it can't be a glyph
    
    if(!item_meta.hasDisplayName()) return null; // If the item does not have a display name, it can't be a glyph
    if(!item_meta.hasLore()) return null; // If the item does not have lore, it can't be a glyph
    List<String> lore = item_meta.getLore(); // Getting the lore
    if(lore.size() < 2) return null; // Lore needs more than just one line
    
    // Parsing the type line
    String type_line = ChatColor.stripColor(lore.get(0)).toLowerCase().replace("glyph of ", "").replace("lv. ", "");
    String[] split_type_line = type_line.split(" "); // Splitting the lore into tokens
    ...
    if(rarity == null) return null; // No rarity no glyph
    
    // Parsing the element
    GlyphElement element = GlyphElement.valueOf(element_string.toUpperCase());
    if(element == null) return null; // No element no glyph
    ...
    for(int i = 1; i < lore.size(); i++)
    {
      ...
      String token_last = s_line[s_line.length - 2]; // second to last token (the experience type)
      int experience = 0;
      try{experience = Integer.parseInt(token_first);}catch(NumberFormatException e){;}
    ...
    for(Attribute a : attributes) glyph.addAttribute(a);
  ...
  {
    if(!this.experience.containsKey(type)) return 0;
   ...
   */
  public void setExperience(Map<String, Integer> experience){this.experience = new HashMap<>(experience);}
   ...
   */
  public void setExperience(String type, int experience){this.experience.put(type, experience);}
  /**Adds the map of experience types and amounts to the current amount of experience in this glyph.
   * If the glyph does not contain any experience, this is equivalent to "setExperience";
   * 
  ...
  public void addExperience(Map<String, Integer> experience)
  {
    ...
    for(String s : experience.keySet())
    {
      ...
      if(this.experience.containsKey(s)) current_experience = this.experience.get(s);
  ...
  {
    int current_experience = 0;
    if(this.experience.containsKey(type)) current_experience = this.experience.get(type);
    this.experience.put(type, experience + current_experience);
  }
  /**Will reset all experience that is currently stored in this glyph
   * This will result in an empty hashmap of exprience values and will not remember what types
   * of experience this glyph had previously had.
   */
  public void resetExperience(){this.experience = new HashMap<String, Integer>();}
  public boolean canLevel()
  {
    Map<String, Integer> level_experience = this.getExperienceToLevel();
    for(String type : level_experience.keySet())
    {
      if(!this.experience.containsKey(type)) return false;
      if(this.experience.get(type) < level_experience.get(type)) return false;
    }
    return true;
  }
  /**Will attempt to levelup the glyph
   * If the glyph does not have enough experience to levelup then the attempt will fail
   * and return false
   * 
   ...
   * Experience types will be discluded in this map if they have a value of 0 or below
   * 
  ...
  public Map<String, Integer> remainingExperience()
  {
    ...
    {
      if(this.experience.containsKey(t)) remain_exp.put(t, required_exp.get(t) - this.experience.get(t));
      else remain_exp.put(t, required_exp.get(t));
      if(remain_exp.get(t) <= 0) remain_exp.remove(t);
    }
    return remain_exp;
  }
  ...
  public int remainingExperience(String type)
  {
    ...
    if(!required_exp_map.containsKey(type)) return -1;
    int required_exp = required_exp_map.get(type);
    if(!this.experience.containsKey(type)) return required_exp;
   ...
   * By contract this itemstack should return an equal glyph object in value when using
  ...
  public ItemStack getItemStack()
  {
    ...
    {
      int exp_amount = 0;
      if(this.experience.containsKey(type)) exp_amount = this.experience.get(type);
      int req_amount = exp_to_level.get(type);
      String exp_line = "";
      if(req_amount > exp_amount) exp_line += "" + ChatColor.GRAY + exp_amount;
      else exp_line += "" + ChatColor.GREEN + exp_amount;
